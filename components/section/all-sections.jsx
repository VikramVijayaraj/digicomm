...
import { getLatestProductsByCategory } from "@/lib/db/products";
...
export default async function AllSections() {
  const latestEbooks = await getLatestProductsByCategory(
    "de015cc7-3ff3-4f5d-858c-749c7e2850ce",
    8,
  );
  const latestDigitalArt = await getLatestProductsByCategory(
    "4606326d-95b0-4018-b744-91d295ac1c56",
  );
  const latestSoftwareTools = await getLatestProductsByCategory(
    "08c92b9d-ca17-49b9-a9e2-7d181288367c",
  );
  const latestMusicAudio = await getLatestProductsByCategory(
    "98a1040e-8b8e-45ad-864e-1857b7e90a2e",
  );
  const latestTemplatesThemes = await getLatestProductsByCategory(
      ...
      {sections.map(({ data, heading }) => (
        <section key={heading}>
          <div className="flex justify-center items-center gap-2 text-xl lg:text-3xl text-center pb-5 md:pb-10">
            <Link href={`/categories/${data[0].categories.slug}`}>
