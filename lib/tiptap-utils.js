import { uploadBlogImageAction } from "@/actions/blog-actions";
...
export const handleImageUpload = async (file, onProgress, abortSignal) => {
  ...
  if (onProgress) onProgress({ progress: 10 }); // Fake start

  try {
    const optimizedFile = await optimizeImage(file, "blogImage");
    const newFileName = formatFileName(file.name, "webp");

    if (onProgress) onProgress({ progress: 50 }); // Midway

    const formData = new FormData();
    formData.append("file", optimizedFile, newFileName);

    // Call the Server Action
    const publicUrl = await uploadBlogImageAction(formData);

    if (onProgress) onProgress({ progress: 100 }); // Complete
  ...
  // const fileName = `blog-images/${file.name}_${uuid()}`;
