...
export async function createSeller(userEmail, shopDetails) {
  ...
  const sellerId = await newShopDetails(data.id, shopDetails);
  return sellerId;
}
...
export async function getShopDetails(email) {
  ...
  // const { data, error } = await supabase.rpc("get_shop_details1", {
  //   p_email: email,
  // });
...
export async function createSellerOrders(sellerId, orderId) {
  const { error } = await supabaseAdmin.from("seller_orders").insert([
    {
      seller_id: sellerId,
      order_id: orderId,
    },
