...
export async function newShopDetails(userId, shopDetails) {
  try {
    await sql`
      INSERT INTO sellers (user_id, shop_name, shop_description, shop_logo, shop_slug)
      VALUES (${userId}, ${shopDetails.name}, ${shopDetails.description}, ${shopDetails.logo}, ${shopDetails.slug})
    `;
  } catch (error) {
    console.error(error);
    throw new Error("Something went wrong. Try again!");
